...
char* ast_to_str(AST* expr)
{
    ...
    case AST_BINARY: {
        return strf(
            "%s %s %s", ast_to_str(expr->Binary.lhs), token_kind_to_str(expr->Binary.op), ast_to_str(expr->Binary.rhs));
    }
...
char* ast_to_json(AST* expr)
{
    ...
    case AST_VARIABLE_DECL: {
        result = strf("{\"%s\": {\"name\": \"%s\", \"type\": \"%s\", \"value\": %s}}", ast_kind_to_str(expr->kind),
            expr->Variable_Decl.name, type_to_str(expr->Variable_Decl.type), ast_to_json(expr->Variable_Decl.value));
    } break;
    case AST_CONSTANT_DECL: {
